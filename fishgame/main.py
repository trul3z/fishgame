 ...
 # Use the enemy's attack message if available, otherwise use default
    ...
    def generate_random_name(self):
        ...
        first_names = [
            ...
            "Whale", "Shark", "Ray", "Eel", "Crab", "Lobster", "Shrimp", "Kelp"
        ]
        ...
        last_names = [
            ...
            "Lighthouse", "Portside", "Starboard", "Windward", "Leeward", "Offshore"
        ]
    ...
    def open_inventory_window(self):
        ...
        for item in self.player.inventory:
            if hasattr(item, 'actual_size'):  # It's a fish
                fish_listbox.insert(tk.END, f"{item.name} ({item.actual_size}in) - {item.get_sell_value()}g")
                fish_count += 1
...
💡 Luck Effects:
• Better chance for rare fish catches
• Improved fishing rarity selection"""
    ...
    def catch_fish(self, location):
        ...
        return f"🐟 Caught a {caught_fish.name} ({caught_fish.actual_size} inches)! Food value: {caught_fish.food_value} energy"
    ...
    def catch_gear(self, location):
        """Find gear while fishing"""
        if not hasattr(self, 'player') or self.player is None:
            return
        found_gear_data = random.choice(self.gear_data['gear'])
        found_gear = Gear(found_gear_data)
        self.player.add_gear(found_gear)
        return f"⚔️ Found {found_gear.name}! {found_gear.description}"
